...
class RadarPlatformSimulation extends Simulation {

  private val conf = ConfigFactory.parseResources("conf/test.conf")
  ...
  private val topics = csv("data/topics.csv")
  ...
  setUp(
    backendCheck.inject(atOnceUsers(numberOfParticipants))
  ).protocols(httpConf)
